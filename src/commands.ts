...
export async function runStep1(
  ...
  try {
    ...
    if (platform === 'darwin') {
      ...
      await sh(`rm -rf /tmp/sessionmanager-bundle.zip /tmp/sessionmanager-bundle`)
    } else {
...
// ── Pre-flight Checks ──────────────────────────────────
// Run before the wizard starts to validate the system is ready.
...
async function checkOSVersion(): Promise<PreflightResult> {
  if (!isDarwin()) {
    ...
    return { name: 'OS', status: 'ok', message: release.stdout.trim() || 'Linux' }
  }
  const verResult = await sh('sw_vers -productVersion')
  const version = verResult.stdout.trim()
  const major = parseInt(version.split('.')[0] || '0')
  if (major < 13) {
    return { name: 'macOS', status: 'warn', message: `${version} (< 13 Ventura, some tools may not work)` }
  }
 ...
 * privileges` dialog (see `sudoSh`) — both refuse for non-admin users,
 * and Homebrew aborts outright with "Don't run this as root!" under sudo.
...
async function checkDiskSpace(): Promise<PreflightResult> {
  ...
  if (dfResult.code !== 0) {
    return { name: 'Disk space', status: 'warn', message: 'Could not determine free space' }
  }
  ...
  if (availGB < 20) {
    return { name: 'Disk space', status: 'warn', message: `${availGB} GB free (< 20 GB recommended)` }
  }
...
async function checkNetwork(): Promise<PreflightResult> {
  ...
  if (result.code !== 0) {
    return { name: 'Network', status: 'fail', message: 'Cannot reach github.com (check your internet connection)' }
  }
