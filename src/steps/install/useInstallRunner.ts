...
export function useInstallRunner(): InstallRunner {
  ...
  const doneTasks = tasks.filter(
    (t) =>
      t.status === "done" || t.status === "skipped" || t.status === "failed",
  );
  ...
  useEffect(() => {
    ...
    for (const task of runnableTasks) {
      ...
      setTasks((prev) =>
        prev.map((t) =>
          t.id === task.id ? { ...t, status: "running", currentSubtask: 0 } : t,
        ),
      );
      ...
      const onProgress: ProgressCallback = (
        ...
        setTasks((prev) =>
          prev.map((t) =>
            t.id === task.id
              ? { ...t, currentSubtask: subtaskIndex, detail }
              : t,
          ),
        );
      };
