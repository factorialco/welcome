    ...
    >
      ...
      {phase === "version-manager" && (
        <Box flexDirection="column" gap={1}>
          ...
          <Text dimColor>
            This manages Ruby, Node.js, Python, and Rust versions.
          </Text>
          <Box marginLeft={2}>
          ...
          >
            <Box flexDirection="column">
              <Text dimColor>
                Plugins to install: rust, ruby, nodejs, python
              </Text>
              <Text dimColor>
                Versions will be read from the repo's .tool-versions file
              </Text>
            </Box>
          </Box>
        </Box>
          ...
          <Box marginLeft={2} flexDirection="column">
            {CLI_OPTIONS.map((opt, i) => (
              <Box key={opt.key} gap={1}>
                <Text
                  color={cliCursor === i ? BRAND_COLOR : "white"}
                  bold={cliCursor === i}
                >
                  {cliCursor === i ? ">" : " "}
                </Text>
                <Text
                  color={
                    config.agenticClis.includes(opt.key) ? "green" : "gray"
                  }
                >
                  {config.agenticClis.includes(opt.key) ? "[x]" : "[ ]"}
                </Text>
                <Text
                  color={cliCursor === i ? "white" : "gray"}
                  bold={cliCursor === i}
                >
                  {opt.label}
                </Text>
      ...
      {phase === "editors" && (
        <Box flexDirection="column" gap={1}>
          ...
          <Text dimColor>
            We'll install recommended extensions for each selected editor.
            Select none to skip editor setup.
          </Text>
          ...
          <Box marginLeft={2} flexDirection="column">
            {EDITOR_OPTIONS.map((opt, i) => (
              <Box key={opt.key} gap={1}>
                <Text
                  color={editorCursor === i ? BRAND_COLOR : "white"}
                  bold={editorCursor === i}
                >
                  {editorCursor === i ? ">" : " "}
                </Text>
                <Text
                  color={config.editors.includes(opt.key) ? "green" : "gray"}
                >
                  {config.editors.includes(opt.key) ? "[x]" : "[ ]"}
                </Text>
                <Text
                  color={editorCursor === i ? "white" : "gray"}
                  bold={editorCursor === i}
                >
                  {opt.label}
                </Text>
