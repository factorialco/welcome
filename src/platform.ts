...
export function getPlatform(): Platform {
  ...
  throw new Error(
    `Unsupported platform: ${p}. Only macOS and Linux are supported.`,
  );
}
...
export async function getLinuxDistro(): Promise<LinuxDistro> {
  ...
  try {
    ...
    const id =
      idLine?.split("=")[1]?.replace(/"/g, "").trim().toLowerCase() ?? "";
...
export async function buildPackageInstallPlan(
  brewNames: string[],
  ...
  const specialMap = isApt
    ? SPECIAL_INSTALL_COMMANDS_APT
    : SPECIAL_INSTALL_COMMANDS_PACMAN;
...
export async function getNativeInstallCommand(
  packages: string[],
): Promise<string> {
  if (isDarwin()) {
...
export async function getLibBuildFlags(
  ...
  const zstdLibDir =
    (await shFn("pkg-config --libs-only-L zstd 2>/dev/null")).stdout.trim() ||
    "-L/usr/lib";
  const zstdIncDir =
    (await shFn("pkg-config --cflags-only-I zstd 2>/dev/null")).stdout.trim() ||
    "-I/usr/include";
    ...
    switch (app.brewCask) {
      ...
      case "ngrok":
        if (await isAptBased()) {
          ...
          plan.commands.push(
            "yay -S --noconfirm ngrok || paru -S --noconfirm ngrok",
          );
        }
        break;
      default:
